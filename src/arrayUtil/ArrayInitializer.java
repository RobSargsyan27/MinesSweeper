    ...
    for(int i = 0; i < board.length; i++){
        ...
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[i].length; j++) {
                if(board[i][j].equals("X")){
                    int[] coordinate = new int[]{i, j};
                    assignNumberToCellByMines(board, coordinate);
                }
            }
        }
        ...
        private static void assignNumberToCellByMines(String[][] board, int[] coordinate){
        int row = coordinate[0];
        int column = coordinate[1];
            ...
            if(i > -1 && i < board.length){
                for(int j = (column - 1); j < (column + 2); j++){
                    if(j > -1 && j < board[i].length){
                        if((row != i || column != j) && !board[i][j].equals("X")){
                            int currentValue = Integer.parseInt(board[i][j]);
                            board[i][j] = String.valueOf(currentValue + 1);
                        }
                    }
                }
            }
